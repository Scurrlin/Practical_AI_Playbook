...
When something goes wrong with AI-assisted development, the instinct is to keep asking for fixes — describe the problem, ask for a fix, get another broken version, describe that problem, ask for another fix. The session gets longer. The context gets polluted. The code gets worse.
...
**What it means:**
The session is polluted. More quick fixes will not help — they will compound the damage. The feature needs to be rebuilt in a clean context, not patched further.
