...
Read `memory.md` first. Then read project-level AI instruction and context files if they exist:

- `AGENTS.md`
- Relevant files in `context/`
- `context.md`

Do not scan or read other files beyond this list unless the user or `AGENTS.md` points to them. For `context/`, read only the files needed to restore the project state, usually `project-overview.md`, `architecture.md`, `build-plan.md`, `progress-tracker.md`, and any standards relevant to the next task. Build the most complete picture possible from what is available.
...
Do not start building. Do not assume the developer wants to continue immediately. Summarise what was restored so the developer can verify the AI agent understood correctly.
